...
import static c.n.t.a.s.j.a.utils.MessageBundleUtils.getMessage;
import static org.apache.commons.lang3.StringUtils.isBlank;

/**
 * @ClassName Validators
 * @Author Colain.Yin
 * @date 2025/9/11 14:14
 */
public class Validators {
    ...
    public static List<String> validatePageRequest(PageRequest page) {
        List<String> validationErrs = new ArrayList<>();
        if (page == null){
            validationErrs.add(getMessage("parameter.not.null", "page"));
        } else {
            if (page.getCur() < Constants.MIN_PAGE_NO){
                validationErrs.add(getMessage("parameter.pageNo.length"));
            }
            if (page.getSize() < Constants.MIN_PAGE_SIZE || page.getSize() > Constants.MAX_PAGE_SIZE){
